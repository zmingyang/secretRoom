...
outputIOBoard::outputIOBoard(QGraphicsView *graphWidget, QString s, QMenu *ContextMenu):
...
void outputIOBoard::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
...
QRectF outputIOBoard::boundingRect() const
{
    int rad = 10;
...
void outputIOBoard::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
//    scene()->clearSelection();
//    setSelected(true);
//    myContextMenu->exec(event->screenPos());

    outputBoardProperty dlg(this);
    dlg.exec();
...
void outputIOBoard::DoUpdate( std::string key, int PinVal)
{
